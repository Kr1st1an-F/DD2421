...
def mlParams(X, labels, W=None):
    ...
    #     Xclass = X[indices]
    #     mu[i] = np.mean(Xclass, axis=0)
    #     sigma[i] = np.cov(Xclass, rowvar=False)
    # # ==========================
...
def classifyBayes(X, prior, mu, sigma):
    ...
    for i in range(Nclasses):
        for j in range(Npts):
            logProb[i][j] = -(1/2)*np.log(np.linalg.det(sigma[i])) - (1/2)* np.dot(X[j]-mu[i], np.dot(np.linalg.inv(sigma[i]), np.transpose(X[j]-mu[i]))) + np.log(prior[i])   
    # ==========================
    # - 1/2 * 1x2 * 2x2 --> 1x2 * 2x1 --> 1x1 scalar
...
N = classifyBayes(X, prioR, mu, sigma)
...
def trainBoost(base_classifier, X, labels, T=10):
    ...
    for i_iter in range(0, T):
        ...
        # ==========================
        labelVote_Correct = np.where(labels == vote)[0]
        labelVote_Incorrect = np.where(labels != vote)[0]
        
        error_t = np.sum(wCur)
        for i in labelVote_Correct:
            error_t -= wCur[i]
        
        alpha = (1/2) * (np.log(1-error_t)-np.log(error_t))
        alphas.append(alpha) # you will need to append the new alpha

        old_wCur = wCur
        # Maybe incorrect np.sum(olf_wCur)
        for i in labelVote_Correct:
            wCur[i] = old_wCur[i] * np.exp(-alpha)

        for i in labelVote_Incorrect:
            wCur[i] = old_wCur[i] * np.exp(alpha)
